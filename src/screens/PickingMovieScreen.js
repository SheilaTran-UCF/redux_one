...
export default function PickingMovieScreen({navigation}) {
  const movieSearch = useSelector(state => state.movies.searchMovies);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <Title h1 style={{alignSelf: 'center', fontWeight: 'bold'}}>
          Search Movies
        </Title>
        <View style={styles.searchContainer}>
          ...
          <Ant
            onPress={() => {}}
            style={{zIndex: 99, right: 40, top: 4}}
            name="search1"
            size={30}
          />
        </View>
        <FlatList
          ...
          data={movieSearch}
          renderItem={({item}) => (
            <MoviePickerRendering navigation={navigation} movie={item} />
          )}
